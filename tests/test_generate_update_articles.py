...
from pipeline import generate_update_articles, ArticleData  # Импортируйте функцию и класс из вашего модуля

class TestGenerateUpdateArticles(unittest.IsolatedAsyncioTestCase):
    async def test_generate_update_articles(self):
        # Создание mock-объектов
        article = ArticleData(title="Тестовая статья", content="Это тестовая статья", short_description=None)
    ...
    async def test_generate_update_articles_with_fast_command(self):
        ...
        # Тестирование функции
        result = await generate_update_articles(article, fast_command="Тестовая быстрая команда")
