...
if __name__ == "__main__":
    ...
    async def test():
        ...
        headlines_us = await fetch_top_headlines(
            top_headlines_url, NEWS_API_KEY, language="en", country="us", limit=3
        )
        ...
        sources = await fetch_sources(
            sources_url, NEWS_API_KEY, language="en", category=Categories.GENERAL.value
        )
        print("\nAvailable sources:")
        for src in sources:
        ...
        headlines_bbc = await fetch_top_headlines(
            top_headlines_url, NEWS_API_KEY, source="bbc-news", limit=3
        )
